<?php
	...
	class cor {
		...
		public function insertCor() {
		    ...
		    $lista->bind_param( "s", $this->descricaoCor );
            $lista->execute();
		...
		public function updateCor() {
		    ...
		    $lista->bind_param( "sd", $this->descricaoCor, $this->codigoCor  );
            $lista->execute();
			desconectaBanco($conn);
		}
		...
		public function selectAllCor() {
			$conn = conectaBanco();
			$lista = $conn->query("Select CodigoCor, DescricaoCor from cor order by DescricaoCor;");
			desconectaBanco($conn);
			return $lista;
		}
	}
